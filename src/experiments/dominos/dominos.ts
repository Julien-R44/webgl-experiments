...
function init() {
    ...
    var back = new THREE.Mesh(gem, new THREE.MeshBasicMaterial({ map: gradTexture([[0.75, 0.6, 0.4, 0.25], ['#1B1D1E', '#3D4143', '#72797D', '#b0babf']]), side: THREE.BackSide, depthWrite: false, fog: false }));
...
async function generateCubesAlongLine(lineGeometry: BufferGeometry) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        const domino = createDomino(
            ...
            new THREE.Euler(
                0,
                points[i + 1] ? Math.atan2(points[i + 1].x - points[i].x, points[i + 1].z - points[i].z) : 0,
                0
            )
        )
